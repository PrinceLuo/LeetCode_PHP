...
function findCommonCharacters($arr){
    ...
    for($i=0; $i<strlen($fir_word); $i++){
        
    }
    for($i=1; $i<$words_num; $i++){
        $cur_num = strlen($arr[$i]);
        $min_length = $min_length<$cur_num?$min_length:$cur_num;
    }
    return $output;
}
...
$output_num = findCommonCharacters(["bella","label","roller"]);
